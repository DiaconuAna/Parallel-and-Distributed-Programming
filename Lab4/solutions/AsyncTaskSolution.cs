...
public class AsyncTaskSolution
{
    ...
    private static async void StartClient(string host, int id)
    {
        ...
        // connect
        await Connect(requestSocket);
        
        // request data
        await Send(requestSocket, DataParser.GetRequestString(requestSocket.Hostname, requestSocket.Endpoint));

        // receive response
        await Receive(requestSocket); 
        
        Console.WriteLine("Connection {0} > Content length is:{1}", requestSocket.Id, DataParser.GetLengthContent(requestSocket.Response.ToString()));
    ...
    private static async Task Connect(MySocket state)
    {
    ...
    private static async Task Send(MySocket state, string data)
    {
    ...
    private static async Task Receive(MySocket state)
    {
